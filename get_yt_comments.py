...
key = 'AIzaSyC-9lNetr41_MI1jDaVTwIcWbO_cxTYMv4' #replace with your youtube data api key
videoId = '53me-ICi_f8&ab'
channelId = ''
...
# vid_id to table name
def get_vid_title(vidid):
    # VideoID = "t_KdbASIkB8&ab"
...
if __name__ == '__main__':
    uroger_comments = get_comments()
    df = pd.DataFrame(uroger_comments)
    print(df.shape)
    print(df.head())
    df['date'] = pd.to_datetime(df['date'], errors='coerce')
    df['just_date'] = df['date'].dt.date
    df.to_csv('./uroger_comments.csv')